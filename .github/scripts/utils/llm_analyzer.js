...
async function callLLM(systemPrompt, userPrompt, maxTokens = 8000) {
  const resp = await fetch(ARK_API, {
    ...
    body: JSON.stringify({
      model: MODEL, stream: false, max_output_tokens: maxTokens,
      input: [
  ...
  if (!Array.isArray(output)) return output?.content?.[0]?.text || JSON.stringify(output);
  return output.map(o => o.content?.map(c => c.text).join("")).join("\n");
}
